...
public class Desencriptaciones {
	...
	public static final String CIPHER_DES="des";  // Problema
	...
	public static final String CIPHER_TRIPLEDES="tripledes"; // Problema
	...
	public static void main(String[] args) {
		...
		switch (encriptacion) {
		case 3, 8:// Para ejecutar el codigo correspondiente a los algorimos DES y 3DES - Base64
			try {
				if (encriptacion == 3) {
					// desencryptDES("BBBkkg5GAaMrahelifvf6GhwoD26fn/03Gf7vE7HA8A=", "12345678");
					desencryptDES(args[0], args[1]);
				} else {
					// desencryptTripleDES("BBBkkg5GAaMrahelifvf6GhwoD26fn/03Gf7vE7HA8A=", "123456781234567812345678");
					desencryptTripleDES(args[0], args[1]);
				}
		...
		default: // Hexadecimal
			...
			String texto_cifrado_hex = args[0];
			byte[] texto_cifrado = HexFormat.of().parseHex(texto_cifrado_hex);
			byte[] clave = args[1].getBytes();
			short modo_hash = Short.valueOf(args[3]);
			short modo_PAD = Short.valueOf(args[4]);
			
			// String texto_cifrado_hex = "3cbc417355dcebf7";
			// byte[] texto_cifrado = HexFormat.of().parseHex(texto_cifrado_hex);
			...
			try {
				byte[] texto_descifrado = decrypt(texto_cifrado, clave);
				String texto_plano = new String(texto_descifrado);
	...
	public static void desencryptDES(String textoCifrado, String key) throws InvalidKeyException, UnsupportedEncodingException {
		byte[] encryptedText;
		byte[] decryptedText;
		...
		//text
		encryptedText = Base64Coder.decodeLines(textoCifrado);
		decryptedText = new byte[textoCifrado.length()];
		
		//Iterate over the byte arrays by 8-byte blocks and decrypt.
		for (int i=0; i<Array.getLength(encryptedText); i+=8) {
			des.decrypt(encryptedText, i, decryptedText, i, keyObject, 8);
		}
	...
	public static void desencryptTripleDES(String textoCifrado, String key) throws InvalidKeyException, UnsupportedEncodingException {
		byte[] encryptedText;
		byte[] decryptedText;
