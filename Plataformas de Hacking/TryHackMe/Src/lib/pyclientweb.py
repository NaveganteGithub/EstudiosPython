...
def get(url: str = "https://www.python.org/", tiempo_espera_limite: int = 7, verbose: bool = False):
    ...
    El metodo get() devuelve la siguiente informacion en forma de diccionario:
    ...
    # saltara alguna de estas excepciones dependiendo del caso
        ...
        # En caso de que NO SALTE NINGUNA EXCEPCION, y tener el modo verbose activado
    ...
    # Si la conexion se establecido correctamente pediremos la informacion del servidor
    # a traves del Socket
    if respuesta.status < 300:
        remote_address = str(conexion.sock).split('\'')[-2]
        remote_port = str(conexion.sock).split('\'')[-1].split(")")[0].split(", ")[1]
